...
Regex *make_regex(char *pattern, int flags) {
    ...
    int ret = regcomp(regex, pattern, flags);//Copied from other doc
    if (ret) {
...
int regex_match(Regex *regex, char *s) {
    ...
    if (!ret) {//If successfull matching
      return 1;
    } else if (ret == REG_NOMATCH) {//If not matching
      return 0;
    } else {//Error
        char msgbuf[100];//Allocate space for potential error
...
void regex_free(Regex *regex) {
    //Called the inbuilt free for regex
    regfree(regex);
}
