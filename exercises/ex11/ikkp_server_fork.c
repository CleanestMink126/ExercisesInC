...
int say(int socket, char *s)
{
    ...
    printf("%s",'p');//This will cause a seg fault
    //In this file the segfault wont bring down the server
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        int f = fork();
        //be open to another request if we are the original server
        //I verified it work by accepting two difference connections and different
        //Terminals
        if(f != 0){
          close(connect_d);
          continue;
