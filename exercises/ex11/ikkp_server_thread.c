...
int say(int socket, char *s)
{
    ...
    printf("%s",'p');//causes seg segfault
    //with threads, it will bring down the server
    if (res == -1)
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        //technically we should probably put in a mutex but it's one line of
        //critical section so it's probably fine for a hw
